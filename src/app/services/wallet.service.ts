...
export class WalletService {
  ...
  private walletSummarySubject = new BehaviorSubject<WalletSummary>({
    ...
    totalEarnings: 45678.5,
    monthlyRevenue: 15250.0,
    revenueChange: 12.5,
    nextPayoutAmount: 8500.0,
    ...
    return this.http.get<any>(`${this.apiUrl}/Booking/ByWorkshop/${workshopProfileId}`).pipe(
      switchMap((response: any) => {
        ...
        const transactionObservables = completedBookings.map((booking: any) => {
          ...
          const workshopServiceCall = this.http
            .get<any>(`${this.apiUrl}/WorkshopService/${booking.workshopServiceId}`)
            .pipe(
              switchMap((wsResponse: any) => {
                ...
                if (serviceId) {
                  return this.http
                    .get<any>(`${this.apiUrl}/Service/${serviceId}`)
                    .pipe(catchError(() => of({ serviceName: 'Service' })));
                }
          ...
          return forkJoin({
            ...
            carOwner: this.http
              .get<any>(`${this.apiUrl}/CarOwnerProfile/by-booking/${booking.id}`)
              .pipe(catchError(() => of({ fullName: 'Customer' }))),
            car: this.http
              .get<any>(`${this.apiUrl}/Car/${booking.carId}`)
              .pipe(catchError(() => of({ make: '', model: 'Vehicle' }))),
          }).pipe(
