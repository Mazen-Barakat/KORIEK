...
type IndicatorType = 'ACService' | 'CarLicenseAndEnsuranceExpiry' | 'GeneralMaintenance' | 'OilChange' | 'BatteryHealth' | 'TireChange';
...
export class CarDetailsComponent implements OnInit {
  ...
  mapIndicatorFromAPI(apiIndicator: any): ConditionIndicator {
    ...
    const remaining = apiIndicator.indicatorType === 'CarLicenseAndEnsuranceExpiry'
    ...
    return {
      ...
      // UI fields
      name: indicatorNames[apiIndicator.indicatorType as IndicatorType] || apiIndicator.indicatorType,
  ...
  getAverageCost(): number {
    ...
    return Math.round(this.getTotalCost() / history.length);
  }

  // Schedule Maintenance
  scheduleMaintenanceDialog(): void {
    console.log('Open schedule maintenance dialog');
    // Open dialog or navigate to scheduling page
  }
  ...
  getSeasonalRecommendation(): { title: string; description: string } | null {
    ...
    if (month >= 11 || month <= 2) {
      return {
        ...
        description: 'Consider checking antifreeze levels, battery health, and tire condition for winter driving.'
      };
  ...
  getIndicatorHistory(indicator: ConditionIndicator): IndicatorHistory[] {
    ...
    const baseCondition = indicator.status === 'Normal' ? 85 :
                          indicator.status === 'Warning' ? 65 : 45;
  ...
  toggleIndicatorReminder(indicator: ConditionIndicator): void {
    ...
    console.log(`Reminder ${indicator.reminderEnabled ? 'enabled' : 'disabled'} for ${indicator.name}`);
  ...
  shareIndicator(indicator: ConditionIndicator): void {
    const shareText = `${indicator.name} - Due: ${indicator.nextInspectionDate}, Remaining: ${this.getRemainingDisplay(indicator)}`;
  ...
  getTimelineEvents(): TimelineEvent[] {
    ...
    this.vehicle.conditionIndicators.forEach(indicator => {
      if (indicator.nextInspectionDate) {
        ...
        const daysFromToday = Math.floor((eventDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
  ...
  getCalendarDays(): CalendarDay[] {
    ...
    for (let day = 1; day <= lastDay.getDate(); day++) {
      ...
      const services = this.vehicle?.conditionIndicators.filter(indicator => {
        if (indicator.nextInspectionDate) {
  ...
  getUpcomingCosts(): Array<{ period: string; services: string[]; amount: number }> {
    ...
    this.vehicle.conditionIndicators.forEach(indicator => {
      if (indicator.nextInspectionDate && indicator.lastServiceCost) {
        const serviceDate = new Date(indicator.nextInspectionDate);
        const monthsDiff = (serviceDate.getFullYear() - today.getFullYear()) * 12 +
  ...
  getUpcomingServicesCount(): number {
    ...
    return this.vehicle?.conditionIndicators.filter(indicator => {
      if (indicator.nextInspectionDate) {
        const serviceDate = new Date(indicator.nextInspectionDate);
